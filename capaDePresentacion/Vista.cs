    ...
    public class Vista{
        ...
        public T TryObtenerEntradaDeTipo<T>(string prompt){
            ...
            while (true){
                ...
                // c# throw new Exception: Lanzamos una Excepción para indicar que el usuario ha cancelado la entrada (en este caso escribir fin)

                if (input.ToLower().Trim() == CANCELINPUT) throw new Exception("Entrada cancelada por el usuario");
                try{

                    // c# Reflexion -> comprueba que es el tipo de dato que debe de recibir (int, parseado obviamente)
                    ...
                    if(input!="")MuestraError($"Error: '{input}' no reconocido como entrada permitida"); //si se introduce algo erróneo enseña el mensaje y si se da a enter sin escribir volverá a pedir el dato
                }
            }
        }
